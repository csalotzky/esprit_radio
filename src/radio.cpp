...
void Radio::UltimateTune(uint8_t setFreqMode, bool resetRds, bool resumeRdsLoop, bool resumeSignalLoop, bool guiMainClear, bool guiRdsClear, bool guiCacheRds, bool guiBandPreset) {
    ...
    if (currentStation.Source == SOURCE_LW || 
        ...
        currentStation.Source == SOURCE_FM) SetFrequency(currentStation.Frequency,currentStation.Source==SOURCE_FM,setFreqMode);
    
    /* HANDLE TASKS */
    if (resetRds) rds.Reset();
    if (resumeRdsLoop && currentStation.Source == SOURCE_FM) vTaskResume(LoopRdsDecodeHandle);
    if (resumeSignalLoop && (
        currentStation.Source == SOURCE_FM || 
        currentStation.Source == SOURCE_LW || 
        currentStation.Source == SOURCE_MW || 
        currentStation.Source == SOURCE_SW)) vTaskResume(LoopSignalStrengthHandle);
    
    /*  GUI */
    if (guiMainClear) GuiRef->ClearMain(true);

    if (guiBandPreset) GuiRef->DisplaySourcePreset(currentStation.Source, currentPreset);
    if (guiRdsClear) GuiRef->ClearRds();
    if (guiCacheRds) {
        if (currentStation.RdsPi) GuiRef->DisplayRdsPi(currentStation.RdsPi,0);
...
void Radio::SetDefaultStation(Station *station, Sources source) {
    station->Frequency = 
        source == SOURCE_FM ? Regions[config.Region].FmMin : 
        source == SOURCE_LW ? Regions[config.Region].LwMin : 
        source == SOURCE_MW ? Regions[config.Region].MwMin : 
        source == SOURCE_SW ? Regions[config.Region].SwMin : 0;
}
...
void Radio::ReadLastStation() {
    ...
    if (!ReadLastSourceSPIFFS(&currentStation.Source)) currentStation.Source = SOURCE_FM;
...
void Radio::SwitchSource(Sources source, bool performTune) {  
    ...
    if (currentStation.Source == SOURCE_LW || 
        ...
        currentStation.Source == SOURCE_SW ||
        currentStation.Source == SOURCE_FM || 
        currentStation.Source == SOURCE_WEB) SaveSourceLastStationSPIFFS(currentPreset, currentStation);

    // Store new source id
    if (source != SOURCE_STANDBY) SaveLastSourceSPIFFS(source);
...
void Radio::TuneEnterSrc(Sources source, int8_t freqNum, bool forceTune) {
    ...
    if (!forceTune) {
        ...
        */  
        bandMin = 
            source==SOURCE_FM ? Regions[config.Region].FmMin : 
            source==SOURCE_LW ? Regions[config.Region].LwMin : 
            source==SOURCE_MW ? Regions[config.Region].MwMin : Regions[config.Region].SwMin;
        bandMax = 
            source==SOURCE_FM ? Regions[config.Region].FmMax : 
            source==SOURCE_LW ? Regions[config.Region].LwMax : 
            source==SOURCE_MW ? Regions[config.Region].MwMax : Regions[config.Region].SwMax;

        if (enterFreq < 10) {
            uint16_t minFirstDigit = bandMin;
            uint16_t maxFirstDigit = bandMax;
            while (minFirstDigit >= 10) minFirstDigit /= 10; 
            while (maxFirstDigit >= 10) maxFirstDigit /= 10;
    ...
    if ((source == SOURCE_FM ? enterFreq*10 : enterFreq)*10 > bandMax || forceTune) {
        if (IsValidFreq((source == SOURCE_FM ? enterFreq*10 : enterFreq),source)) {
            ...
            if(currentStation.Source == source) TuneFrequency(source == SOURCE_FM ? enterFreq*10 : enterFreq);
            else TuneFrequency(source == SOURCE_FM ? enterFreq*10 : enterFreq, source);
            enterFreq = 0;
            enterSource = SOURCE_STANDBY;
            return;
        }
...
int8_t Radio::CalcStep(bool minStep, uint16_t freq, Sources source) {
    if (source == SOURCE_FM) return minStep ? Regions[config.Region].FmMinStep : Regions[config.Region].FmMaxStep;
    if (source == SOURCE_LW) return minStep ? Regions[config.Region].LwMinStep : Regions[config.Region].LwMaxStep;
    if (source == SOURCE_MW) return minStep ? Regions[config.Region].MwMinStep : Regions[config.Region].MwMaxStep;
    if (source == SOURCE_SW) return minStep ? Regions[config.Region].SwMinStep : Regions[config.Region].SwMaxStep;
    return -1;
}
...
uint16_t Radio::AddStepToFrequency(uint16_t freq, uint8_t step, bool up, Sources source) {
    // Calc new frequency
    uint16_t tempFreq;
    if (up) tempFreq = (freq + step) / step * step;
    else tempFreq = freq % step != 0 ? freq / step * step : freq - step;

    // Check if valid and correct
    if (source == SOURCE_FM)
    {
       if (tempFreq <  Regions[config.Region].FmMin) tempFreq = Regions[config.Region].FmMax;
       else if(tempFreq > Regions[config.Region].FmMax) tempFreq = Regions[config.Region].FmMin;
    }
    if (source == SOURCE_LW)
    {
       if (tempFreq <  Regions[config.Region].LwMin) tempFreq = Regions[config.Region].LwMax;
       else if(tempFreq > Regions[config.Region].LwMax) tempFreq = Regions[config.Region].LwMin;
    }
    else if (source == SOURCE_MW)
    {
       if (tempFreq <  Regions[config.Region].MwMin) tempFreq = Regions[config.Region].MwMax;
       else if(tempFreq > Regions[config.Region].MwMax) tempFreq = Regions[config.Region].MwMin;
    }
    else if (source == SOURCE_SW)
    {
       if (tempFreq <  Regions[config.Region].SwMin) tempFreq = Regions[config.Region].SwMax;
       else if(tempFreq > Regions[config.Region].SwMax) tempFreq = Regions[config.Region].SwMin;
    }
...
bool Radio::IsValidFreq(uint16_t freq, Sources source) {
    ...
    if (source==SOURCE_MW) return freq >= Regions[config.Region].MwMin && freq <= Regions[config.Region].MwMax;
    if (source==SOURCE_SW) return freq >= Regions[config.Region].SwMin && freq <= Regions[config.Region].SwMax;
    return false;
}
                ...
                if (status.IsDataAvailable) {
                    ...
                    if (GetRdsData(&data))
                    {
                        ...
                        if (radioRef->rds.ReadPi(&piTemp, &piSafeTemp)) {
                            ...
                            if (piSafeTemp || radioRef->config.PiMode == RDS_DX || (radioRef->config.PiMode == RDS_MIXED && !radioRef->currentStation.RdsPi)) radioRef->GuiRef->DisplayRdsPi(piTemp,piSafeTemp);
                        } 
                        ...
                        if (radioRef->rds.ReadPs(psTemp, &psSafeTemp)) {
                            ...
                            if (psSafeTemp && radioRef->currentStation.RdsPs[0] == '\0') strcpy(radioRef->currentStation.RdsPs, psTemp);
                            // Display if: safe || DX mode || No safe ps was sent yet (MIXED mode)
                            if (psSafeTemp || radioRef->config.PsMode == RDS_DX || (radioRef->config.PsMode == RDS_MIXED && radioRef->currentStation.RdsPs[0] == '\0')) radioRef->GuiRef->DisplayRdsPs(psTemp, psSafeTemp);
                        }
                        if (radioRef->rds.ReadRt(rtTemp)) {
                            radioRef->GuiRef->DisplayRdsRt(rtTemp);
                        }
                    } 
                    else Serial.println("TUNER | Error while fetching RDS DATA");
                }

            }
            else Serial.println("TUNER | Error while fetching RDS STATUS");
        }
        vTaskSuspend(NULL);
    }
}
...
void Radio::Seek(void *params) {
    while(true) {
        ...
        while (!found && radioRef->seekState != SEEK_STOP) {
            ...
            if (i==SEEK_MAX_ATTEMPT && !found) found = sigSum/(i) >= radioRef->config.SeekLevel && usnSum/(i) <= radioRef->config.SeekUsn;
        }
