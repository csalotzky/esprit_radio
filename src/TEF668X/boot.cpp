...
//#include "Tuner_Patch_Lithio_V102_p222.h"
...
static uint16_t Tuner_Patch_Load(const unsigned char * pLutBytes, uint16_t size)
{
	...
	while(size)
	{
		...
		if(1 != (r = TunerBufferWrite(buf, len+1)))
		{
			break;
		}
	}
...
uint16_t Tuner_Table_Write(const unsigned char * tab)
{
...
//load patch table2 
	else if(tab[1] == INIT_FLAG_PATCH2)
	{
		...
		return TunerBufferWrite((unsigned char *)&tab[1], tab[0]);
}
