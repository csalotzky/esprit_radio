...
#define SEMAPHORE_MAX_ATTEMPT 1
...
{
    return TunerSetCmd(TEF665X_MODULE_AUDIO,
...
{
    uint8_t buf[12];
    return TunerGetCmd(TEF665X_MODULE_FM,
                       TEF665X_Cmd_Get_RDS_Data, NULL, 0);
}
...
int TunerSetCmd(TEF668x_MODULE module, uint8_t cmd, int len, ...)
{
    ...
    {
        uint8_t attempt = 0;
        while (xSemaphoreTake(i2cSemaphore, (TickType_t)100) != pdTRUE && attempt != SEMAPHORE_MAX_ATTEMPT)
            attempt++;
...
int TunerGetCmd(TEF668x_MODULE module, uint8_t cmd, uint8_t *receive, int len)
{
    ...
    if (i2cSemaphore != NULL)
    {
        ...
        if (attempt != SEMAPHORE_MAX_ATTEMPT)
        {
            ...
            if (success)
                success = TunerBufferRead(receive, len) == 1;
